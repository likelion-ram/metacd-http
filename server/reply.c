...
static enum http_rc_e
_reply_json (struct http_reply_ctx_s *rp, int code, const gchar * msg,
	GString * gstr)
{
...
static enum http_rc_e
_reply_notfound_error (struct http_reply_ctx_s *rp, GError * err)
{
	return _reply_json (rp, 404, "Not found", _create_status_error (err));
}

static enum http_rc_e
_reply_forbidden_error (struct http_reply_ctx_s *rp, GError * err)
{
	return _reply_json (rp, 403, "Forbidden", _create_status_error (err));
}
